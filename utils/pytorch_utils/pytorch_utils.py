...
class RSConv_bkk(nn.Module):
    ...
    def __init__(
        ...
        if first_layer:
            ...
            self.bn_mapping = nn.BatchNorm2d(math.floor(C_out))
        self.activation = activation
        self.relation_prior = relation_prior
        self.first_layer = first_layer
        self.mapping_func1 = mapping[0]
    ...
    def forward(self, input): # input: (B, 3 + 3 + C_in, npoint, centroid + nsample)
        ...
        coor = input[:, 0:3, :, :]
        x = torch.cat([x, coor], dim=1)
        x = self.xyz_raising(x)
